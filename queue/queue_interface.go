...
type InterfaceQueueConnectionPool interface {
	...
	// DeleteMessage
	DeleteMessage(queueName string, ID uint64) error

	// ReturnMessage
	ReturnMessage(queueName string, ID uint64, delay time.Duration) error

	// GiveupMessage
	GiveupMessage(queueName string, ID uint64) error
}
